...
class CustomUserAdmin(UserAdmin, admin.ModelAdmin):
    ...
    form = CustomUserChangeForm
    model = CustomUser
    list_display = ('username', 'email', 'faculty_type_display')
    list_filter = ('email', 'username',)
    search_fields = ('email',)
    ordering = ('email',)

    fieldsets = (
        ("Basic Profile Details", {'fields': ('email', 'username')}),
    ...
    add_fieldsets = (
        ("Basic Profile Details", {
            'classes': ('wide',),
            'fields': ('email', 'username', 'mobile_no', 'groups'),
        }),
    )
    ...
    def get_queryset(self, request):
        ...
        elif current_user.groups.filter(name='Faculty Admin').exists():
